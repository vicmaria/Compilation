...
let struct_array_char_name = ref "arraychar"
...
let type2c
  ...
  | TypChar -> fprintf out "char"
  | TypBool -> fprintf out "int"
  | TypIntArray -> fprintf out "struct %s*" !struct_array_name
  | TypCharArray -> fprintf out "struct %s*" !struct_array_char_name
...
let expr2c
  ...
  let rec expr2c out e =
    ...
    | EArrayCharAlloc e ->
       fprintf out "(void*)({ char %s = %a; \
                    if (%s < 0) exit(1); \
                    struct %s* res = tgc_alloc(({ extern tgc_t gc; &gc; }), sizeof(struct %s)); \
                    res->array = (char*) tgc_calloc(({ extern tgc_t gc; &gc; }), %s, sizeof(char)); \
                    res->length = %s; res; })"
         !name1
         expr2c e
         !name1
         !struct_array_char_name
         !struct_array_char_name
         !name1
         !name1
