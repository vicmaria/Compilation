...
let rec compatible (typ1 : typ) (typ2 : typ) (instanceof : identifier -> identifier -> bool) : bool =
  ...
  | TypCharArray, TypCharArray -> true
...
and typecheck_expression (cenv : class_env) (venv : variable_env) (vinit : S.t)
  ...
  | EConst (ConstFloat _) -> TypFloat

  | EConst (ConstChar _)  -> TypChar
  ...
  | EBinOp (OpEq, e1, e2) ->
    ...
    (**call compatible*)
    if not (compatible type2 type1 instanceof || compatible type1 type2 instanceof) then
      error e
        (sprintf "Type mismatch, expected %s, got %s" (type_to_string type1) (type_to_string type2))
    else
      TypBool
  ...
  | EArrayGet (earray, eindex) ->
    ...
    let t =  typecheck_expression cenv venv vinit instanceof earray in
    match t with
    |TypIntArray -> TypInt
    |TypCharArray -> TypChar
    |_ ->
      error e
          (sprintf "Type mismatch array")
  ...
  | EArrayLength earray ->
    begin
      ...
      match t with
        ...
        |_ -> 
          error earray
          (sprintf "Type mismatch array lenght")
        end
  ...
  | IArraySet (earray, eindex, evalue) ->
    ...
    match t with
      |TypIntArray ->
        ...
        vinit
      |TypCharArray ->
          typecheck_expression_expecting cenv venv vinit instanceof TypChar eindex;
          typecheck_expression_expecting cenv venv vinit instanceof TypChar evalue;
          vinit
      |_ -> 
        error earray
        (sprintf "Type mismatch array")
